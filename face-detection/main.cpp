...
int main() {
  ...
  // 其中 x 是最后保留的框（检测到人脸）的个数，最后一维存放每次检测结果的数据，不同的人脸模型的输出可能是不一样的
  // 最后一维每一列的含义：
  // 列 0：物体存在的置信度 TODO
  // 列 1：包围盒的置信度 TODO
  // 列 2：检测到的人脸置信度
  // 列 3：人脸边框左上角坐标 x 在整张图片中的比例
  // 列 4：人脸边框左上角坐标 y 在整张图片中的比例
  // 列 5：人脸边框右下角坐标 x 在整张图片中的比例
  // 列 6：人脸边框右下角坐标 y 在整张图片中的比例

  // 所以 detection.size[2] 是检测到的对象的数量 TODO 有误，detection.size[3] 是每次检测的结果的数据（人脸边界框数据和置信度）
  // 类似于：
  ...
  // Draw the rectangles
  // std::cout << detection_mat.rows << std::endl;
  std::cout << detection.size[2] << std::endl;
  for (int i = 0; i < detection_mat.rows; i++) {
    // std::cout << detection_mat.at<float>(i, 0) << ", " << detection_mat.at<float>(i, 1) << std::endl;
